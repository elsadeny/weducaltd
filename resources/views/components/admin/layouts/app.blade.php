...
<aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-64 bg-navy flex flex-col transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
    ...
    <div class="flex items-center space-x-3 px-6 py-5 border-b border-white/10">
        ...
        <div>
            <span class="font-bold text-white text-base tracking-tight">Weduca<span class="text-teal">Apply</span></span>
            <div class="text-xs text-amber-400 font-semibold uppercase tracking-wider">Admin Panel</div>
        </div>
    </div>
    ...
    <div class="px-4 py-4 border-b border-white/10">
        <div class="flex items-center space-x-3 bg-white/5 rounded-xl px-3 py-3">
            <div class="w-9 h-9 rounded-full bg-amber-500 flex items-center justify-center text-white font-bold text-sm shrink-0">
                {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
            </div>
            <div class="overflow-hidden">
                <div class="text-white text-sm font-semibold truncate">{{ Auth::user()->name }}</div>
                <div class="text-amber-400 text-xs font-semibold uppercase tracking-wider">Administrator</div>
            </div>
        </div>
    </div>
...
<div class="flex-1 flex flex-col lg:ml-64 min-h-screen">
    ...
    <header class="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-20">
        ...
        <div class="flex items-center space-x-3">
            <span class="bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full">Admin</span>
        </div>
    </header>
